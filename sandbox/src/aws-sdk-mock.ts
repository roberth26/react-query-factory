...
const ALL_INSTANCES: Instance[] = Array.from({ length: 95 }, (_, i) => ({
  ...
  Tags: [
    { Key: 'Name', Value: `prod-worker-${String(i + 1).padStart(3, '0')}` },
  ],
...
export class EC2Client {
  ...
  send(
    cmd: DescribeAvailabilityZonesCommand,
    opts?: { abortSignal?: AbortSignal },
  ): Promise<DescribeAvailabilityZonesResult>;
  send(
    cmd: DescribeInstancesCommand,
    opts?: { abortSignal?: AbortSignal },
  ): Promise<DescribeInstancesResult>;
  send(
    cmd: DescribeInstanceTypesCommand,
    opts?: { abortSignal?: AbortSignal },
  ): Promise<DescribeInstanceTypesResult>;
  send(
    cmd: StopInstancesCommand,
    opts?: { abortSignal?: AbortSignal },
  ...
  > {
    ...
    if (cmd instanceof DescribeInstanceTypesCommand) {
      ...
      const nextToken =
        start + MaxResults < ALL_INSTANCE_TYPES.length
