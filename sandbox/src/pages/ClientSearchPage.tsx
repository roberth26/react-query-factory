...
function ClientSearchPage() {
  ...
  return (
    <SpaceBetween size="m">
      <TextContent>
        <p>
          When the API has no server-side search or filter endpoint, you can
          crawl pages client-side and stop the moment your condition is
          satisfied. <code>findInstance</code> inherits{' '}
          <code>describeInstances</code>&#39;s fetch and pagination config,
          overriding only <code>shouldFetchNextPage</code> to stop once the
          target appears in the accumulated results. Repeat searches cost zero
          API calls — TanStack serves them from cache. The UX implication is a
          fast, cache-aware search that only touches as many pages as it needs.
        </p>
      </TextContent>
      ...
      <Container
        header={
          <Header
            variant="h2"
            description="Crawl stops the moment the target instance is found. Re-searching the same ID costs zero API calls."
          >
