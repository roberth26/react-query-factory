...
const describeInstancesViaPaginator = queryFactory({
	...
	queryFn: (params: { pageSize?: number }) =>
		paginateDescribeInstances({ client: ec2, pageSize: params.pageSize ?? 20 }, {}),
	shouldFetchNextPage: (instances: Instance[] | undefined, opts: { minResults?: number }) =>
		opts.minResults == null || (instances?.length ?? 0) < opts.minResults,
	reduce: (acc: Instance[] | undefined, page: DescribeInstancesResult): Instance[] => [
		...(acc ?? []),
...
export async function loader() {
	await queryClient.prefetchQuery(
		describeInstancesViaPaginator({ pageSize: 20 }, { minResults: Number.MAX_SAFE_INTEGER }),
	);
	return null;
}
...
function PaginatorPage() {
	...
	return (
		<SpaceBetween size="m">
			<TextContent>
				<p>
					When a <code>queryFn</code> returns an <code>AsyncIterable</code>, the factory
					detects it and drives the crawl with <code>for await...of</code>, calling{' '}
					<code>shouldFetchNextPage</code> after each yielded item to decide whether to keep
					going. The iterator owns cursor management, so <code>getNextPageParam</code>,{' '}
					<code>initialPageParam</code>, and token threading are not required. AWS SDK v3
					paginator functions — <code>paginateDescribeInstances</code> and its equivalents —
					return async iterables and plug in directly. Any other source of{' '}
