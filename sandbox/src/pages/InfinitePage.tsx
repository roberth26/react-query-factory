...
// data.pages → Instance[][]  (one flat array per UI page)`;
	...
	return (
		<SpaceBetween size="m">
			<TextContent>
				<p>
					AWS pagination is server-driven: <code>MaxResults</code> caps each API call, but the
					server may return fewer items regardless. When a UI page needs 20 rows but the server
					only gives 5 at a time, traditional infinite query setups leave the user with ragged
					pages unless you add a manual crawl loop between <code>fetchNextPage</code> calls. This
					factory&#39;s <code>shouldFetchNextPage</code> accepts a per-call{' '}
					<code>minResults</code> option that drives exactly that loop internally — each virtual
					page accumulates API responses until it reaches the target count, then hands off a
					complete, uniform page to the UI. The UX benefit is consistent page sizes: users advance
