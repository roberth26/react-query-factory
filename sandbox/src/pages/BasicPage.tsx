...
function BasicPage() {
	...
	return (
		<SpaceBetween size="m">
			<TextContent>
				<p>
					<code>DescribeAvailabilityZones</code> returns a single, non-paginated response — the
					simplest shape the AWS SDK produces. This example shows the minimal factory: just a{' '}
					<code>queryKey</code> and a <code>queryFn</code>, with no pagination or accumulation
					options needed. Before this library, even simple queries required boilerplate to wire up
					a consistent cache key and typed parameters. The factory encapsulates that once, and
					every call site gets type-safe options for free. From a UX perspective, a single-call
					query means the table either loads instantly or shows a spinner briefly — there is no
