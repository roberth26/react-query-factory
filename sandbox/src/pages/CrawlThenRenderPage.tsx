...
export async function loader() {
	await Promise.all([
		queryClient.prefetchQuery(describeInstanceTypes({ MaxResults: 10 }, { minResults: Number.MAX_SAFE_INTEGER })),
		...
		<SpaceBetween size="m">
			<TextContent>
				<p>
					...
					types populate the Select while instances load the table. When the user changes the
					type filter, only the instance query re-crawls — the type list is already cached. From
					a UX standpoint, the Select is disabled until the full crawl completes, ensuring the
					user always sees every available option.
				</p>
			</TextContent>
<ExpandableSection headerText="Factory code" variant="container">
			...
			<Table
				...
				header={
					<Header variant="h2" counter={instances ? `(${instances.length})` : undefined}>
						{selectedTypes[0]
