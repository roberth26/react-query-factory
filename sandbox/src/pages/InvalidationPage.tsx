...
function InvalidationPage() {
	...
	return (
		<SpaceBetween size="m">
			<TextContent>
				<p>
					Mutations like <code>StopInstances</code> don&#39;t update TanStack Query&#39;s cache
					automatically — you must invalidate the relevant queries after a successful response.
					With raw <code>useQuery</code>, keeping every related key in sync across multiple
					mutation handlers is tedious and fragile: add a new derived query and you must remember
					to add its key to every <code>onSuccess</code>. The factory solves this at two levels
					of precision. <strong>Broad:</strong> calling with no arguments returns the namespace
					key — a single <code>invalidateQueries(describeInstances())</code> marks every variant,
					every param set, and every child factory stale simultaneously.{' '}
					<strong>Scoped:</strong> child keys are ordered{' '}
					<code>[...parentNS, params, ...childSegments]</code>, so the parent called with params
					is always a prefix of its children for those same params —{' '}
					<code>invalidateQueries(describeInstances(params))</code> busts only the queries for
					that specific call site, leaving unrelated cache entries untouched.
				</p>
			</TextContent>
