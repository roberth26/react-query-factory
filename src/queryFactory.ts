...
function resolveKey(namespace: QueryKey, params: unknown, crawlOptions?: Record<string, unknown>): QueryKey {
  ...
  const defined = Object.fromEntries(Object.entries(crawlOptions).filter(([, v]) => v !== undefined));
...
function wrapGetNextPageParam<TData, TPageParam, TSelected>(
  getNextPageParam: GetNextPageParamFunction<TPageParam, TData>,
  shouldFetchNextPage: (combined: TSelected | undefined, crawlOptions: Record<string, unknown>) => boolean,
  ...
  return (lastPage, allPages, lastPageParam, allPageParams) => {
    const combined = reduce
      ? allPages.reduce<TSelected | undefined>((acc, page) => reduce(acc, page), undefined)
...
function buildCrawlingQueryFn<TData, TPageParam, TSelected>(
  queryFn: (ctx: QueryFunctionContext<any, any>) => TData | Promise<TData>,
  getNextPageParam: GetNextPageParamFunction<TPageParam, TData>,
  initialPageParam: TPageParam,
  shouldFetchNextPage: ((combined: TSelected | undefined, crawlOptions: Record<string, unknown>) => boolean) | undefined,
  reduce: ((accumulator: TSelected | undefined, page: TData) => TSelected) | undefined,
  crawlOptions: Record<string, unknown>,
): (ctx: QueryFunctionContext) => Promise<TSelected | TData[]> {
  return async (context) => {
    ...
    while (true) {
      ...
      const page = await queryFn({ ...context, pageParam: currentParam } as any);
      pages.push(page);
...
function buildInfiniteCrawlingQueryFn<TData, TPageParam, TSelected>(
  ...
  reduce: (accumulator: TSelected | undefined, page: TData) => TSelected,
  crawlOptions: Record<string, unknown>,
): (ctx: QueryFunctionContext<any, TPageParam>) => Promise<CrawlEnvelope<TSelected, TPageParam>> {
  return async (context) => {
    const pages: TData[] = [];
    const pageParams: TPageParam[] = [];
...
function buildFactory(cfg: NormalizedConfig): QueryFactory<any, any, any, any, any, any> {
  const {
    queryKey: namespace,
    queryFn: rawQueryFn,
    select,
  ...
  const factory = function (params: any, crawlOptions: Record<string, unknown> = {}) {
    ...
    const resolvedQueryFn = hasCrawling
      ? buildCrawlingQueryFn(boundQueryFn!, getNextPageParam, initialPageParam, shouldFetchNextPage, reduce, crawlOptions)
      : boundQueryFn;
  ...
  factory.infinite = function (params: any, crawlOptions: Record<string, unknown> = {}) {
    ...
    const queryKey = resolveKey([...resolveKey(namespace, undefined), 'infinite'], params, crawlOptions);
    ...
    if (hasInfiniteCrawling) {
      ...
      const envelopeGetNextPageParam = (envelope: CrawlEnvelope<any, any>) =>
        envelope.nextPageParam;

      const envelopeSelect = (data: { pages: CrawlEnvelope<any, any>[]; pageParams: any[] }) => ({
        ...data,
    ...
    const infiniteGetNextPageParam =
      getNextPageParam && shouldFetchNextPage
        ? wrapGetNextPageParam(getNextPageParam, shouldFetchNextPage, crawlOptions, reduce)
        : (getNextPageParam ?? (() => undefined));
...
export function queryFactory<
  ...
  TCrawlOptions extends Record<string, unknown> = Record<string, unknown>,
>(
  config: QueryFactoryConfig<TParams, TData, TError, TSelected, TPageParam, TCrawlOptions>,
): QueryFactory<TParams, TData, TError, TSelected, TPageParam, TCrawlOptions>;
...
>(
  ...
  config: Omit<
    QueryFactoryConfig<TChildParams, TData, TError, TChildSelected, TPageParam, TCrawlOptions>,
    'queryKey' | 'getNextPageParam' | 'getPreviousPageParam' | 'initialPageParam' | 'shouldFetchNextPage' | 'reduce'
  > & {
    queryKey?: QueryKey;
...
export function queryFactory(
  ...
  if (childConfig !== undefined && typeof configOrParent === 'function') {
    ...
    const childNamespace = childConfig.queryKey
      ? (Array.isArray(childConfig.queryKey) ? childConfig.queryKey : [childConfig.queryKey])
      : [];
    const composedNamespace: QueryKey = [...parentCfg.queryKey, ...childNamespace];
    ...
    const crawling = hasNewQueryFn
      ...
      : {
          getNextPageParam: childConfig.getNextPageParam ?? parentCfg.getNextPageParam,
          getPreviousPageParam: childConfig.getPreviousPageParam ?? parentCfg.getPreviousPageParam,
          initialPageParam: childConfig.initialPageParam !== undefined
            ? childConfig.initialPageParam
            : parentCfg.initialPageParam,
          shouldFetchNextPage: childConfig.shouldFetchNextPage ?? parentCfg.shouldFetchNextPage,
          reduce: childConfig.reduce ?? parentCfg.reduce,
        };

    const {
      queryKey: _k, queryFn: _f, select: _s,
      getNextPageParam: _g, getPreviousPageParam: _gp,
      initialPageParam: _ip, shouldFetchNextPage: _sfnp, reduce: _c,
